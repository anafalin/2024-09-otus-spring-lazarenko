...
public class BookServiceImplTest {
    ...
    void findById_whenBookExists_thenEmptyBookOptional() {
        ...
        // testing
        assertThrows(EntityNotFoundException.class, () -> bookService.findById(notExistIdBook));
    }
    ...
    void save_whenCorrectRequest_thenSavesBook() {
        ...
        BookDto insertedBook = bookService.save(
                ...
                List.copyOf(expectedBook.getGenres().stream().map(Genre::getId).collect(Collectors.toSet())));
    ...
    void save_whenAuthorNotExist_thenEntityNotFoundException() {
        ...
        assertThrows(EntityNotFoundException.class,
                () -> bookService.save(title, authorNotExistId, List.of(genre.getId())),
    ...
    void save_whenOneOfGenreIdIsNotExist_thenEntityNotFoundException() {
        ...
        List<Long> genresIds = List.of(dbGenres.get(0).getId(), 100L);
    ...
    void update_whenCorrectRequest_thenUpdatedBook() {
        ...
        BookDto existingBook = bookService.findById(expectedBook.getId());
        assertAll(
                () -> assertThat(existingBook.getId()).isEqualTo(updatableBook.getId()),
                () -> assertThat(existingBook.getTitle()).isEqualTo(updatableBook.getTitle())
        );

        // testing
        BookDto updatedBook = bookService.update(updatableBook.getId(), expectedBook.getTitle(),
                expectedBook.getAuthor().getId(), List.of(dbGenres.get(3).getId()));
    ...
    void update_whenOneOfGenreIdIsNotExist_thenEntityNotFoundException() {
        ...
        List<Long> genresIds = List.of(updatableBook.getGenres().get(0).getId(), 100L);
    ...
    void deleteById_whenBookExist_thenBookWasDeleted() {
        ...
        BookDto existingBook = bookService.findById(deltableBook.getId());
        assertAll(
                () -> assertThat(existingBook.getId()).isEqualTo(deltableBook.getId()),
                () -> assertThat(existingBook.getTitle()).isEqualTo(deltableBook.getTitle())
        );
        ...
        // verification
        assertThrows(EntityNotFoundException.class, () -> bookService.findById(deltableBook.getId()));
    }
}